...
<fmt:bundle basename="pagecontent" prefix="albumView.">
    ...
    <head>
        ...
        <script>
            <jsp:include page="/js/modal/albumTrack.js"/>
        </script>
    ...
    <div class="content">
        <div class="view-header">
            ...
            <c:if test="${sessionScope.user.role.value eq 'admin'}">
                ...
                <div id="addTrackToAlbum" class="modal"
                     <c:if test="${requestScope.addTrackMessage ne null}">style="display: block;"</c:if>>
                    <div class="form-style">
                        ...
                        <form name="addTrack"
                              action="${pageContext.servletContext.contextPath}/controller?command=submitTrack"
                              method="post">
                            <input type="hidden" id="albumId" name="album_id">
                            <input type="hidden" id="artistName" name="artist">
                            <label>
                                <input type="text" name="title" pattern="['A-Za-z0-9,.' ]{1,32}"
                                       title="<fmt:message key="artistTitleDesc"/>"
                                       placeholder="<fmt:message key="title"/>" required/>
                            </label>
                            ...
                            <label>
                                <input type="text" name="price" pattern="^[0-9]+(\.[0-9]{2})?$"
                                       title="<fmt:message key="priceDesc"/>" placeholder="<fmt:message key="price"/>"
                                       required/>
                            </label>
            ...
            <c:if test="${sessionScope.user.role.value eq 'client'}">
                <div class="icon-button"
                     onclick="window.location='${pageContext.servletContext.contextPath}/controller?command=orderAlbum&albumId=${requestScope.album.id}'">
