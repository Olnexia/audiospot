...
<fmt:bundle basename="pagecontent" prefix="ordering.">
    ...
    <head>
        ...
        <script>
            <jsp:include page="/js/jquery/jquery-3.2.1.min.js"/>
        </script>
        <style>
            <jsp:include page="/css/search.css"/>
        </style>
        <script type="text/javascript">
            <jsp:include page="/js/trackSearch.js"/>
        </script>
    ...
    <div class="content" >
        ...
        <c:if test="${sessionScope.user.role.value eq 'admin'}">
            <div class="icon-button"
                 onclick="window.location='${pageContext.servletContext.contextPath}/controller?command=addTrack'">
                <div class="small add-icon"></div>
                <p class="b-text"><fmt:message key="newTrack"/></p>
            </div>
        </c:if>
        <div class="searchContainer">
            <input class="searchBox" type="search" id="audioSearch" onkeyup="searchTracks()" placeholder="<fmt:message key="search"/>">
            <input type="submit" onclick="searchTracks()" value="Search" class="searchButton">
        </div>
                    ...
                    <tr>
                        ...
                        <td class="manage-buttons">
                            <c:if test="${sessionScope.user.role.value eq 'client'}">
                                <div class="icon-button "
                                     onclick="window.location='${pageContext.servletContext.contextPath}/controller?command=orderTrack&trackId=${track.id}'">
                            ...
                            <c:if test="${sessionScope.user.role.value eq 'admin' and not empty param.audioSetId}">
                                <div class="icon-button "
                                     onclick="window.location='${pageContext.servletContext.contextPath}/controller?command=addToSet&trackId=${track.id}&audioSetId=${param.audioSetId}'">
                                    <div class="small add-icon"></div>
                                    <p class="b-text"><fmt:message key="addToSet"/></p>
                                </div>
                            </c:if>
                            <div class="icon-button "
                                 onclick="window.location='${pageContext.servletContext.contextPath}/controller?command=showComments&trackId=${track.id}'">
